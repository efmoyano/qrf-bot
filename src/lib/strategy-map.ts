...
import { ParticipationRole, Player, Registration, SquadType } from "@prisma/client";
import { db } from "./db.js";
import { formatPower } from "./format.js";

export interface DesertStormBuildingDef {
  id: string;
  name: string;
  shortName: string;
  badge: string;
...
export const DESERT_STORM_BUILDINGS: DesertStormBuildingDef[] = [
  // Center
  { id: "NUCLEAR_SILO", name: "NUCLEAR SILO", shortName: "Nuclear Silo", badge: "SILO", x: 440, y: 340, w: 240, h: 220, color: "#f97316" },
  // Top & Bottom
  { id: "ARSENAL", name: "ARSENAL", shortName: "Arsenal", badge: "ATK", x: 440, y: 70, w: 240, h: 220, color: "#ef4444" },
  { id: "MERCENARY_FACTORY", name: "MERCENARY FACTORY", shortName: "Mercenary Factory", badge: "MERC", x: 440, y: 610, w: 240, h: 220, color: "#eab308" },
  // Left Column (West Flank)
  { id: "INFO_CENTER", name: "INFO CENTER", shortName: "Info Center", badge: "INFO", x: 70, y: 70, w: 240, h: 160, color: "#06b6d4" },
  { id: "OIL_REFINERY_1", name: "OIL REFINERY 1", shortName: "Oil Refinery 1", badge: "OIL-1", x: 70, y: 250, w: 240, h: 160, color: "#3b82f6" },
  { id: "HOSPITAL_1", name: "HOSPITAL 1", shortName: "Hospital 1", badge: "HOSP-1", x: 70, y: 430, w: 240, h: 160, color: "#10b981" },
  { id: "HOSPITAL_3", name: "HOSPITAL 3", shortName: "Hospital 3", badge: "HOSP-3", x: 70, y: 610, w: 240, h: 160, color: "#10b981" },
  // Right Column (East Flank)
  { id: "HOSPITAL_4", name: "HOSPITAL 4", shortName: "Hospital 4", badge: "HOSP-4", x: 810, y: 70, w: 240, h: 160, color: "#10b981" },
  { id: "HOSPITAL_2", name: "HOSPITAL 2", shortName: "Hospital 2", badge: "HOSP-2", x: 810, y: 250, w: 240, h: 160, color: "#10b981" },
  { id: "OIL_REFINERY_2", name: "OIL REFINERY 2", shortName: "Oil Refinery 2", badge: "OIL-2", x: 810, y: 430, w: 240, h: 160, color: "#3b82f6" },
  { id: "SCIENCE_HUB", name: "SCIENCE HUB", shortName: "Science Hub", badge: "TECH", x: 810, y: 610, w: 240, h: 160, color: "#a855f7" },
];
...
export interface RenderMapOptions {
  eventTitle: string;
  assignments: Record<string, PlayerMapEntry[]>;
  substitutes: PlayerMapEntry[];
}

const SQUAD_BADGE: Record<SquadType, { label: string; color: string; bg: string }> = {
...
export function buildDesertStormMapSvg(options: RenderMapOptions): string {
  const { eventTitle, assignments, substitutes } = options;

  const buildingBoxes = DESERT_STORM_BUILDINGS.map((b) => {
    ...
    const playerRows = assigned.slice(0, 6).map((p, idx) => {
      ...
      return `
        <g transform="translate(${b.x + 12}, ${yOffset})">
          ...
          <text x="46" y="0" fill="#f8fafc" font-size="13" font-weight="600" font-family="system-ui, sans-serif">${safeName}</text>
    ...
    return `
      ...
      <g>
        ...
        <rect x="${b.x}" y="${b.y}" width="${b.w}" height="${b.h}" rx="8" fill="#111827" fill-opacity="0.9"/>
  ...
  const subRows = substitutes.map((p, idx) => {
    ...
    return `
      <g transform="translate(1130, ${yOffset})">
        ...
        <text x="54" y="0" fill="#f8fafc" font-size="13" font-weight="600" font-family="system-ui, sans-serif">${safeName}</text>
        <text x="268" y="0" text-anchor="end" fill="#fbbf24" font-size="12" font-weight="600" font-family="system-ui, monospace">${p.power}</text>
      </g>
    `;
  }).join("\n");

  const safeTitle = escapeXml(eventTitle.toUpperCase());
  ...
  return `
    <svg xmlns="http://www.w3.org/2000/svg" width="1460" height="880" viewBox="0 0 1460 880">
      ...
      <text x="60" y="30" fill="#94a3b8" font-size="13" font-weight="700" letter-spacing="2" font-family="system-ui, sans-serif">
        DESERT STORM BATTLEFIELD MAP — ${safeTitle}
      </text>
...
export function renderDesertStormStrategyMap(options: RenderMapOptions): Buffer {
  const svg = buildDesertStormMapSvg(options);
...
type RegWithPlayer = Registration & { player: Player };

export async function getEventStrategyData(eventId: string): Promise<{
  assignments: Record<string, PlayerMapEntry[]>;
  substitutes: PlayerMapEntry[];
  unassignedStarters: Array<RegWithPlayer>;
}> {
  ...
  const assignments: Record<string, PlayerMapEntry[]> = {};
  for (const b of DESERT_STORM_BUILDINGS) {
    assignments[b.id] = [];
  }

  const substitutes: PlayerMapEntry[] = [];
  const unassignedStarters: Array<RegWithPlayer> = [];

  for (const r of registrations) {
    const entry: PlayerMapEntry = {
      name: r.player.gameName,
      power: formatPower(r.powerSnapshot),
      squad: r.squadSnapshot,
    };

    if (r.role === ParticipationRole.SUBSTITUTE) {
    ...
    if (r.assignedBuilding && assignments[r.assignedBuilding]) {
      assignments[r.assignedBuilding].push(entry);
    } else {
      unassignedStarters.push(r);
    }
  }
...
export async function autoDistributeStrategy(eventId: string): Promise<number> {
  ...
  // Clear existing assignments for starters
  ...
  for (const r of remaining) {
    assignmentsToMake.push({ id: r.id, building: "NUCLEAR_SILO" });
  }
  ...
  await Promise.all(
    assignmentsToMake.map((a) =>
      db.registration.update({
        where: { id: a.id },
        data: { assignedBuilding: a.building },
      }),
    ),
  );
